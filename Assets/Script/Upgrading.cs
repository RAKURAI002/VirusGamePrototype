...
public class Upgrading : MonoBehaviour
{
    ...
    void GetProductionPoint()
    {
        ...
        if (laborCenter.CharacterInBuilding != null)
        {
            productionPointTemp += laborCenter.CharacterInBuilding[laborCenter.constructionStatus.teamNumber].Characters.Sum(c => ((c.Stats.strength * 0.2f / 8) + (c.Stats.speed * 0.2f / 8) + (c.Stats.crafting * 0.8f / 3)));
        }
